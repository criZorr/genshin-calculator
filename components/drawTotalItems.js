...
let enemiesKeys = [];
let eliteEnemiesKeys = [];

for (let i = 0; i < Object.keys(enemies).length; i++) {
  let names = enemies[i][0];
  names.forEach((e) => enemiesKeys.push(e));
}

for (let i = 0; i < Object.keys(eliteEnemies).length; i++) {
  let names = eliteEnemies[i][0];
  names.forEach((e) => eliteEnemiesKeys.push(e));
}
