...
import getRarity from "./components/getRarity.js";
  ...
  for (let i = 0; i < all.length; i++) {
    if (localUserMaterials[all[i]]) {
      if (localUserMaterials[all[i]] > 0) {
        ...
        fragment.innerHTML = `
          <div class="item-bg ${rarity}" _id="${fixedName}"data-rarity="${rarity}">
            <img src="./assets/materials/${imgName}.webp" alt="${all[i]}" _id="${fixedName}"data-rarity="${rarity}"/>
          </div>
          <h5 class="card-name dark-text" _id="${fixedName}"data-rarity="${rarity}">${number}</h5>
        `;
...
const drawModalElement = (id, rarity) => {
...
const getListElements = () => {
  for (let i = 0; i < all.length; i++) {
    ...
    fragment.innerHTML = `
      <div class="item-bg ${rarity}" _id="${fixedName}" data-rarity="${rarity}">
        <img src="./assets/materials/${tempName}.webp" alt="${all[i]}" _id="${fixedName}" data-rarity="${rarity}"/>
      </div>
      <h5 _name="${tempName}" class="card-name dark-text" _id="${fixedName}" data-rarity="${rarity}"></h5>
    `;
